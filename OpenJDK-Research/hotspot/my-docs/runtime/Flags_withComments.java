[Global flags]
        ...
        uintx LargePageSizeInBytes=0{product}Large page size(0to let VM choose the page size)
        ...
        bool UseSSE42Intrinsics=false{product}SSE4 .2versions of intrinsics
        ...
        uintx SegmentedHeapDumpThreshold=2147483648{develop}Generate a segmented heap dump(JAVA PROFILE 1.0.2format)when the heap usage is larger than this
        ...
        bool ProfilerRecordPC=false{product}Collect ticks for each 16byte interval of compiled code
        ...
        bool UnsyncloadClass=false{diagnostic}Unstable:VM calls loadClass unsynchronized.Custom

class loader must call VM synchronized for findClass and defineClass.
        bool AlwaysLockClassLoader=false{product}Require the VM to acquire the

class loader lock before calling loadClass()even for

class loaders registering as parallel capable
        bool AllowParallelDefineClass                  =false{product}Allow parallel defineClass requests for
...
class loaders registering as parallel capable
        ...
        bool LoadLineNumberTables=true{develop}Tell whether the

class file parser loads line number tables
        bool LoadLocalVariableTables                   =true{develop}Tell whether the

class file parser loads local variable tables
        bool LoadLocalVariableTypeTables               =true{develop}Tell whether the

class file parser loads local variable typetables
        ...
        bool LazyBootClassLoader=true{product}Enable/disable lazy opening of boot
...
class path entries
        ...
        bool PrintFieldLayout=false{notproduct}Print field layout for each
...
intx ContendedPaddingWidth                     =128{product}How many bytes to pad the fields/classes marked@Contended with
        ...
        bool TraceClassInitialization=false{develop}Trace
...
class initialization
     bool TraceExceptions                           =false{develop}Trace exceptions
        ...
        uintx FLSCoalescePolicy=2{product}CMS:aggressiveness level for coalescing,increasing from 0to 4
        ...
        bool CMSClassUnloadingEnabled=true{product}Whether

class unloading enabled when using CMS GC
        uintx CMSClassUnloadingMaxInterval              =0{product}When CMS

class unloading is enabled,the maximum CMS cycle count for which classes may not be unloaded
        ...
        uintx InitiatingHeapOccupancyPercent=45{product}Percentage of the(entire)heap occupancy to start a concurrent GC cycle.It is used by GCs that trigger a concurrent GC cycle based on the occupancy of the entire heap,not just one of the generations(e.g.,G1).A value of 0denotes'do constant GC cycles'.
        bool UseCMSInitiatingOccupancyOnly=false{product}Only use occupancy as a criterion for starting a CMS collection
        uintx CMSIsTooFullPercentage=98{product}An absolute ceiling above which CMS will always consider the unloading of classes when
...
class unloading is enabled
        ...
        bool NeverActAsServerClassMachine=true{pd product}Never act like a server-

class machine
     bool AlwaysActAsServerClassMachine             =false{product}Always act like a server-
...
class machine
 uint64_t MaxRAM                                    =1073741824{pd product}Real memory size(in bytes)used to set maximum heap size
        ...
        uintx AdaptiveSizePolicyWeight=10{product}Weight given to exponential resizing,between 0and 100
        uintx AdaptiveTimeWeight=25{product}Weight given to time in adaptive policy,between 0and 100
        ...
        intx PrefetchCopyIntervalInBytes=-1{product}How far ahead to prefetch destination area(<=0means off)
        intx PrefetchScanIntervalInBytes=-1{product}How far ahead to prefetch scan area(<=0means off)
        intx PrefetchFieldsAhead=-1{product}How many fields ahead to prefetch in oop scan(<=0means off)
        ...
        bool TraceClassLoaderData=false{develop}Trace

class loader loader_data lifetime
        uintx InitialBootClassLoaderMetaspaceSize       =2252800{product}Initial size of the boot

class loader data metaspace
        bool TraceGen0Time                             =false{product}Trace accumulated time for Gen 0collection
        bool TraceGen1Time=false{product}Trace accumulated time for Gen 1collection
        ...
        bool PrintClassHistogramBeforeFullGC=false{manageable}Print a

class histogram before any major stop-world GC
        bool PrintClassHistogramAfterFullGC=false{manageable}Print a

class histogram after any major stop-world GC
        bool PrintClassHistogram=false{manageable}Print a histogram of
...
class instances
    ...
    uintx NumberOfGCLogFiles=0{product}Number of gclog files in rotation(default:0,no rotation)
        uintx GCLogFileSize=0{product}GC log file size(default:0bytes,no rotation).It requires UseGCLogFileRotation
        ...
        bool DelayCompilationDuringStartup=true{develop}Delay invoking the compiler until main application

class is loaded
        bool CompileTheWorld                           =false{develop}Compile all methods in all classes in bootstrap

class path(stress test)
     bool CompileTheWorldPreloadClasses             =true{develop}Preload all classes used by a

class before start loading
        ...
        bool PrintClassStatistics=false{notproduct}Print

class statistics at end of run
        ...
        intx SelfDestructTimer=0{product}Will cause VM to terminate after a given time(in minutes)(0means off)
        intx MaxJavaStackTraceDepth=1024{product}The maximum number of lines in the stack trace for Java exceptions(0means all)
        intx GuaranteedSafepointInterval=1000{diagnostic}Guarantee a safepoint(at least)every so many milliseconds(0means none)
        ...
        uintx CompressedClassSpaceSize=1073741824{product}Maximum size of
...
class pointers are used
        ...
        uintx ThreadSafetyMargin=52428800{product}Thread safety margin is used on fixed-stack LinuxThreads(on Linux/x86 only)to prevent heap-stack collision.Set to 0to disable this feature
     ...
     bool ReplayCompiles=false{develop}Enable replay of compilations from ReplayDataFile
        ...
        intx ReplaySuppressInitializers=2{develop}Control handling of

class initialization during replay:0-don't do anything special; 1 - treat all class initializers as empty; 2 - treat class initializers for application classes as empty; 3 - allow all class initializers to run during bootstrap but     pretend they are empty after starting replay
        ...
        bool CICompilerCountPerCPU=false{product}1compiler thread for log(N CPUs)
        ...
        intx ThreadPriorityPolicy=0{product}0:Normal.VM chooses priorities that are appropriate for normal applications.On Solaris NORM_PRIORITY and above are mapped to normal native priority.Java priorities below NORM_PRIORITY map to lower native priority values.On Windows applications are allowed to use higher native           priorities.However,with ThreadPriorityPolicy=0,VM will not use the highest possible native priority,THREAD_PRIORITY_TIME_CRITICAL,as it may interfere with system threads.On Linux thread priorities are ignored because the OS does not support static priority in SCHED_OTHER scheduling

class which is the only choice for non-root,non-realtime applications. 1:Aggressive.Java thread priorities map over to the entire range of native thread priorities.Higher Java thread priorities map to higher native thread priorities.This policy should be used with care,as sometimes it can cause performance degradation in the application and/or the entire system.On Linux this policy requires root privilege.
        bool ThreadPriorityVerbose=false{product}Print priority changes
        intx DefaultThreadPriority=-1{product}The native priority at which threads run if not elsewhere specified(-1means no change)
        intx CompilerThreadPriority=-1{product}The native priority at which compiler threads should run(-1means no change)
        intx VMThreadPriority=-1{product}The native priority at which the VM thread should run(-1means no change)
        ...
        intx CompileTheWorldStartAt=1{notproduct}First

class to consider when using +CompileTheWorld
        intx CompileTheWorldStopAt=2147483647{notproduct}Last
...
class to consider when using +CompileTheWorld
        ...
        intx Tier23InlineeNotifyFreqLog=20{product}Inlinee invocation(tiers 2and 3)notification frequency
        ...
        intx Tier2CompileThreshold=0{product}threshold at which tier 2compilation is invoked
        intx Tier2BackEdgeThreshold=0{product}Back edge threshold at which tier 2compilation is invoked
        ...
        intx Tier3CompileThreshold=2000{product}Threshold at which tier 3compilation is invoked(invocation minimum must be satisfied
        intx Tier3BackEdgeThreshold=60000{product}Back edge threshold at which tier 3OSR compilation is invoked
        ...
        intx Tier4CompileThreshold=15000{product}Threshold at which tier 4compilation is invoked(invocation minimum must be satisfied
        intx Tier4BackEdgeThreshold=40000{product}Back edge threshold at which tier 4OSR compilation is invoked
        intx Tier3DelayOn=5{product}If C2 queue size grows over this amount per compiler thread stop compiling at tier 3and start compiling at tier 2
        intx Tier3DelayOff=2{product}If C2 queue size is less than this amount per compiler thread allow methods compiled at tier 2transition to tier 3
        intx Tier3LoadFeedback=5{product}Tier 3thresholds will increase twofold when C1 queue size reaches this amount per compiler thread
        intx Tier4LoadFeedback=3{product}Tier 4thresholds will increase twofold when C2 queue size reaches this amount per compiler thread
        ...
        intx Tier0ProfilingStartPercentage=200{product}Start profiling in interpreter if the counters exceed tier 3thresholds by the specified percentage
        ...
        bool UseNewReflection=true{develop}Temporary flag for transition to reflection based on dynamic bytecode generation in 1.4;can no longer be turned off in 1.4JDK,and is unneeded in 1.3JDK,but marks most places VM changes were needed
        bool VerifyReflectionBytecodes=false{develop}Force verification of 1.4reflection bytecodes.Does not work in situations like that described in 4486457or for constructors generated for serialization,so can not be enabled in product.
        ...
        bool DumpSharedSpaces=false{product}Special mode:JVM reads a

class list,loads classes,builds shared spaces,and dumps the shared spaces to a file to be used in future JVM runs
        ...
        uintx SharedBaseAddress=0{product}Address to allocate shared memory region for
...
class data
     bool EnableInvokeDynamic                       =true{diagnostic}support JSR 292(method handles,invokedynamic,anonymous classes
        ...
        bool UseVMInterruptibleIO=false{product}(Unstable,Solaris-specific)Thread interrupt before or with EINTR for I/O operations results in OS_INTRPT.The default value of this flag is true for JDK 6and earlier
        ...
        intx G1SummarizeRSetStatsPeriod=0{diagnostic}The period(in number of GCs)at which we will generate update buffer processing info(0means do not periodically generate this info);it also requires-XX:+G1SummarizeRSetStats
        ...
        uintx G1SATBBufferEnqueueingThresholdPercent=60{product}Before enqueueing them,each mutator thread tries to do some filtering on the SATB buffers it generates.If post-filtering the percentage of retained entries is over this threshold the buffer will be enqueued for processing.A value of 0specifies that mutator threads should not do such filtering.
        ...
        uintx G1ConcRSLogCacheSize=10{product}Log base 2of the length of conc RS hot-card cache.
     ...
     intx G1RSetSparseRegionEntriesBase=4{develop}Max number of entries per region in a sparse table per MB.
        ...
        intx G1MaxVerifyFailures=-1{develop}The maximum number of verification failrues to print.-1means print all.
        ...
        uintx G1ConcRefinementThreads=0{product}If non-0is the number of parallel rem set update threads,otherwise the value is determined ergonomically.
        ...
        bool Use486InstrsOnly=false{ARCH product}Use 80486Compliant instruction subset
