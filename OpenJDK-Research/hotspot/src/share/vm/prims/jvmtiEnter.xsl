...
static jvmtiError JNICALL
  ...
  <xsl:text>      if ((trace_flags &amp; JvmtiTrace::SHOW_ERROR) != 0) {
    ...
    <xsl:template match="outptr|outbuf|allocfieldbuf|ptrtype|inptr|inbuf|vmbuf|allocbuf|agentbuf|allocallocbuf"
                  mode="dochecks">
        <xsl:param name="name"/>
  ...
  if (!rmonitor->is_valid()) {
    ...
    if (thread_oop == NULL) {
        ...
        <xsl:apply-templates select=".." mode="traceError">
            ...
            <xsl:with-param name="extraValue">,
                <xsl:value-of select="$name"/>
            </xsl:with-param>
        </xsl:apply-templates>
        <xsl:text>
  ...
  if (k_oop == NULL) {
    ...
    <xsl:template
            match="jobject|jvalue|jthreadGroup|enum|jchar|jlong|jfloat|jdouble|jlocation|jboolean|char|uchar|size_t|void|struct"
            mode="dochecks">
    </xsl:template>
