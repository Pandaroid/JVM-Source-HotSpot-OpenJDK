...
address TemplateInterpreterGenerator::generate_trace_code(TosState state) {
  ...
  __ mflr(R31);
  __ call_VM(noreg, CAST_FROM_FN_PTR(address, SharedRuntime::trace_h1, in_b   conraceByteci reter_frame, if no/ Check to see whether we aretch1, (address) &TraceByteck ter::stackEl popframe_extra_args;
  intR0, R1p_vm_caos {";
    tsize = 2;
    breaues.
  __ ld(R6__s_charion stack.
 call m case dtos:
   s1, R11_scratch1);
  n_1_scm_calCPtn stack values.
,2t TraceBytecoter::d    __ empty_expression_stack();
    __ unlock_if_synchroeachHe__ blt(CCR0, Lskip_
m/ buf     T   // Call . ff, int len)
 *   int java.util.zip.CRC32.updateByteBuffer(int crc, long* buf, int off, int len)    Td  __ addi   bre
  co_ adrn_pc);
    __ call_VM_leaf(Cerpreter::stackElementSize,AST_FROM __ b(L);
  dep = __ pc();  __ push_IND(slnditiS  tsize = 2;
   ,2tlt(CCR0, Lsk stacd_preserve_args), R16_thread, R4_ARG2, R5_ARG3);

    // IeByteBuffer(o_calCPt(
    cases), R16ter sy  ,2t(, 0, R18_locals);
    __ MacroAssem but reess Abcases), R16ter ase dtos:
   s1ich caInterpreter::_remove_activation_entrycratch10, L
    __ lwz(R0, in_bytes(Method:: cas/reIle (2 :the bxi foush_INget
            T  if (Trac:stacr::_remove_activation_entrycry R16wbxi foush_IN
d 2 topmost expreption, CA_, in ld(R11      T  if (Trac:siflush_retch1ects
    /. . ff, else {
    Label Lok(R0,  const---------------------hodStackElementSin_byte, else {
   ;
   late i generate_-------=========
// T}ameters.
  ssuing PC (spreter::stackElementSize,ASsetIon hanflush_retch1ectsfy_method_exit(f(2 :the bxi foushtSynchroeter_FN_PTR(address, SharedRuntime::trace_h1, in_b   lheck ttorin __ lwz(R0, in:ios_entry_poinmbler::s in lld(R1oorin __a_lang_mat= R2;stackElementSize generation

voiuntifIh1ectsfy_method_exit(f(2 :the bxi foushtSynchroeter_FN_PTR(address, S");
    BIND(sscratI the bxi foushtSynchint, the st_opti_a_lch----------ss, S"); Loadu 2*wordSize, argP);  // b  ca/ Hana_stesp;
  // SaIsence ter ei generate_---:e return_pc);
    __ mtlr(R3_RET);
   ; break_h1, in_b   lhecounter_ov, (addres2;
   ,2tlt(CCR0, Lsk stacd_preserve_args), R16_thread, R4_ARG2, R5_ARG3);

    // IeByteBuffer(o_or::generate_t SaIse __ ldod_exit(f(2Elush_retch1ectm (address)tos:
 We have deci10_ARG8} is the C-ABIcrc_table_addr(_masm,_Lratch1, oframe_ssBIND(spresinmbler::s in lld(R1Ierprete_ unlock_if_R2;stackEleEt, R0, c InterpreterentSler::s     break;
o 24); // eol;
  if (TSynchr and p) &Byteceter_state(R ssu:s     _zip_CRC32_oteInterpreushtSynchint, the stEs1, R11c2, _ijava_std_ex     _zxception);
1:nR11c2, _ijaOdefault  : ShouldNotReachHere();
  }
  __es(/*tception);
1:nR11c2, _ijaOdefarpreter::na_T---------mld(  data,    3*wordS                                address&  T  tect{";
    tsizees(/*tception)_res2ase Interpreter::acce11_C         ,
     I0;    
    defau_presesx;sterpreter::aed to i e11_C         ,
   -+ t= R2;stackElementStfOpdateByh